                                ...
                                <td>
                                    ...
                                    <button type="button" class="btn-action btn-edit" title="Edit Order"
                                        ...
                                        <c:if test="${o.orderStatus != 'Cancelled'}">
                                            <form action="${pageContext.request.contextPath}/dashboard/dashboard/order" method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to cancel Order #O00${o.orderId}?');">
                    ...
                    <nav class="d-flex justify-content-center" style="margin-top: 20px;">
                        <ul class="pagination">
                            <li class="page-item ${currentPage == 1 ? 'disabled' : ''}">
                                <a class="page-link" href="${pageContext.request.contextPath}/dashboard/dashboard/order?action=${not empty searchKeyword ? 'search' : 'list'}${not empty searchKeyword ? '&keyword='.concat(searchKeyword) : ''}&page=${currentPage - 1}">&laquo;</a>
                            </li>

                            <c:forEach begin="${startPage}" end="${endPage}" var="i">
                                <li class="page-item ${currentPage == i ? 'active' : ''}">
                                    <a class="page-link" href="${pageContext.request.contextPath}/dashboard/dashboard/order?action=${not empty searchKeyword ? 'search' : 'list'}${not empty searchKeyword ? '&keyword='.concat(searchKeyword) : ''}&page=${i}">${i}</a>
                                </li>
                            </c:forEach>

                            <li class="page-item ${currentPage == totalPages ? 'disabled' : ''}">
                                <a class="page-link" href="${pageContext.request.contextPath}/dashboard/dashboard/order?action=${not empty searchKeyword ? 'search' : 'list'}${not empty searchKeyword ? '&keyword='.concat(searchKeyword) : ''}&page=${currentPage + 1}">&raquo;</a>
                            </li>
                        </ul>
                    </nav>
                </c:if>    
